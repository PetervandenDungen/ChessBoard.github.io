class Board {
  ...
  public moves: Move;
  public constructor(rows: number, squares: number) {
    this.rows = rows;
    this.squares = squares;
    document.getElementById('board').innerHTML = this.renderBoard();
    this.StartPostition();
  }
  private getCoordinate(index: number, squareInRow: number) {
    enum xCoordinate { A = 1, B, C, D, E, F, G, H };
  ...
  private StartPostition() {
    ...
    //console.log('Pieces Created and placed..');
